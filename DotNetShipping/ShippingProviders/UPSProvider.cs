...
namespace DotNetShipping.ShippingProviders
{
    ...
    public class UPSProvider : AbstractShippingProvider
    {
        ...
        private string ratesUrl = ConfigurationManager.AppSettings["UPSRateUrl"];// "https://www.ups.com/ups.app/xml/Rate";
        //private const string ratesUrl = "https://onlinetools.ups.com/ups.app/xml/Rate";
        ...
        public override void GetRates()
        {
            ...
            {
                SetErrorDescription("Exception: " + ex.Message);
            }
            finally
            {
        ...
        private byte[] buildRatesRequestMessage()
        {
            ...
            if (!string.IsNullOrWhiteSpace(Shipment.OriginAddress.ShipperNumber)) writer.WriteElementString("ShipperNumber", Shipment.OriginAddress.ShipperNumber);
            writer.WriteStartElement("Address");
            if (!string.IsNullOrWhiteSpace(Shipment.OriginAddress.Line1)) writer.WriteElementString("AddressLine1", Shipment.OriginAddress.Line1);
            if (!string.IsNullOrWhiteSpace(Shipment.OriginAddress.City)) writer.WriteElementString("City", Shipment.OriginAddress.City);
            if (!string.IsNullOrWhiteSpace(Shipment.OriginAddress.State)) writer.WriteElementString("StateProvinceCode", Shipment.OriginAddress.State);
            ...
            writer.WriteStartElement("Address");
            if (!string.IsNullOrWhiteSpace(Shipment.DestinationAddress.Line1)) writer.WriteElementString("AddressLine1", Shipment.DestinationAddress.Line1);
            if (!string.IsNullOrWhiteSpace(Shipment.DestinationAddress.City)) writer.WriteElementString("City", Shipment.DestinationAddress.City);
            if (!string.IsNullOrWhiteSpace(Shipment.DestinationAddress.State)) writer.WriteElementString("StateProvinceCode", Shipment.DestinationAddress.State);
        ...
        private void parseRatesResponseMessage(string response)
        {
            ...
            if (xDoc.SelectNodes("/RatingServiceSelectionResponse/Response/Error/ErrorDescription").Item(0) != null)
            {
                SetErrorDescription(xDoc.SelectNodes("/RatingServiceSelectionResponse/Response/Error/ErrorDescription").Item(0).InnerXml);
            }
